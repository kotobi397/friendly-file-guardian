...
interface QueueItem {
  ...
  source_author_image_url?: string | null;
  attempts: number;
  max_attempts: number;
      ...
      fetch(`${supabaseUrl}/functions/v1/bulk-upload-books-ai`, {
        ...
        body: JSON.stringify({
          books: chunk.map((item) => ({
            ...
            source_description: item.source_description ?? undefined,
            source_author_bio: item.source_author_bio ?? undefined,
            source_author_image_url: item.source_author_image_url ?? undefined,
