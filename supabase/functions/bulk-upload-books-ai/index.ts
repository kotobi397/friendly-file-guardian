...
interface InputBook {
  ...
  // بيانات فعلية قادمة من المصدر (نور بوك) — تُفضّل على ما يولّده الذكاء الاصطناعي
  source_description?: string;
  source_author_bio?: string;
  source_author_image_url?: string;
}
...
// ✅ التحقق النهائي: هل الرابط يفتح فعلاً ملفاً صالحاً للقراءة؟
// (يمنع نشر أي كتاب برابط ميت أو ملف تالف — مثلاً عندما يُحذف الأصل بعد فشل العلامة المائية)
async function isReadableFileUrl(url: string): Promise<boolean> {
  if (!url) return false;
...
async function upsertApprovedBook(book: InputBook, meta: AIBookMeta, supabaseClient: any): Promise<BookResult> {
  ...
  // 🔒 لا يُنشر أي كتاب قبل التأكد من أن ملفه يفتح فعلاً
  if (!(await isReadableFileUrl(bookFileUrl))) {
    console.warn(`[AI Bulk] ⚠️ الرابط النهائي غير صالح لـ "${title}" — تجربة الملف الأصلي`);
    if (bookFileUrl !== uploadedBook.url && (await isReadableFileUrl(uploadedBook.url))) {
      bookFileUrl = uploadedBook.url;
    } else {
      return {
        success: false,
        retryable: true,
        title,
        error: "ملف الكتاب غير قابل للقراءة (رابط ميت أو ملف تالف) — لم يُنشر",
      };
    }
  }
  ...
  const payload = {
    ...
    author_image_url: book.source_author_image_url?.trim() || null,
...
// ✍️ إعادة صياغة النص القادم من المصدر بأسلوب مختلف مع الحفاظ على المعنى والحقائق
async function rewriteSourceText(
  text: string,
  kind: "description" | "bio",
  title?: string,
): Promise<string | null> {
  ...
  const systemPrompt =
    kind === "description"
      ? `أنت محرر عربي محترف. أعِد صياغة وصف الكتاب التالي بأسلوبك الخاص:
- احتفظ بكل المعلومات والحقائق كما هي، دون إضافة معلومات مخترعة.
- غيّر التركيب والمفردات وترتيب الأفكار بحيث لا تتطابق أي جملة حرفياً مع النص الأصلي.
- عربية فصحى سلسة، 3-6 جمل، بدون عناوين أو رموز أو ذكر لأي موقع أو مصدر.
أرجع JSON فقط: {"text":"الوصف المُعاد صياغته"}`
      : `أنت محرر عربي محترف. أعِد صياغة نبذة المؤلف التالية بأسلوبك الخاص:
- احتفظ بالحقائق (المولد، الوفاة، التخصص، أهم الأعمال) دون اختراع أي معلومة.
- غيّر الصياغة والمفردات بحيث لا تتطابق أي جملة حرفياً مع النص الأصلي.
- عربية فصحى، 2-5 جمل، بدون ذكر أي موقع أو مصدر.
أرجع JSON فقط: {"text":"النبذة المُعاد صياغتها"}`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const response = await fetch("https://api.mistral.ai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: "user",
              content: `${title ? `عنوان الكتاب: ${title}\n\n` : ""}النص الأصلي:\n${text}`,
            },
          ],
        }),
      });
      ...
      if (out.length >= 40 && out !== text.trim()) return out;
      return null;
    } catch (_) {
      await wait(1000);
    }
...
async function processBooks(books: InputBook[], supabaseClient: any): Promise<BookResult[]> {
  ...
  for (let start = 0; start < books.length; start += MISTRAL_BATCH_SIZE) {
    ...
    // تفضيل البيانات الفعلية القادمة من المصدر (نور بوك) مع إعادة صياغتها
    // حتى لا يكون الوصف مطابقاً حرفياً لوصف المصدر (تجنب المحتوى المكرر)
    await Promise.all(
      batch.map(async (book, i) => {
        if (!metas[i]) return;
        const srcDesc = book.source_description?.toString().trim();
        if (srcDesc && srcDesc.length >= 40) {
          const rewritten = await rewriteSourceText(srcDesc.slice(0, 4000), "description", book.title);
          metas[i].description = (rewritten || srcDesc).slice(0, 4000);
        }
        const srcBio = book.source_author_bio?.toString().trim();
        if (srcBio && srcBio.length >= 40) {
          const rewrittenBio = await rewriteSourceText(srcBio.slice(0, 4000), "bio", book.title);
          metas[i].author_bio = (rewrittenBio || srcBio).slice(0, 4000);
        }
      }),
    );
  ...
  try {
    ...
    const sanitized = books
      .map((book) => ({
        ...
        source_author: book.source_author ? String(book.source_author).trim() : undefined,
        source_description: book.source_description ? String(book.source_description).trim() : undefined,
        source_author_bio: book.source_author_bio ? String(book.source_author_bio).trim() : undefined,
        source_author_image_url: book.source_author_image_url
          ? String(book.source_author_image_url).trim()
          : undefined,
      }))
